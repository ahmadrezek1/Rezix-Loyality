...
export async function isSessionActive(s:Session){const r=await db()`select 1 from auth_sessions where jti=${s.jti} and revoked_at is null and expires_at>now() limit 1`;if(r[0])await db()`update auth_sessions set last_seen_at=now() where jti=${s.jti}`;return !!r[0]}
...
export async function createOneTimeToken(actorId:string,actorType:'manager'|'friseur',purpose:'email_verify'|'password_reset',minutes:number){const token=crypto.randomBytes(32).toString('base64url');const expiresAt=new Date(Date.now()+minutes*60_000);await db()`insert into auth_tokens(id,purpose,actor_id,actor_type,token_hash,expires_at) values (${id('tok')},${purpose},${actorId},${actorType},${h(token)},${expiresAt})`;return token}
export async function consumeOneTimeToken(token:string,purpose:'email_verify'|'password_reset'){const rows=await db()`update auth_tokens set consumed_at=now() where token_hash=${h(token)} and purpose=${purpose} and consumed_at is null and expires_at>now() returning actor_id,actor_type`;return rows[0] as {actor_id:string;actor_type:'manager'|'friseur'}|undefined}
...
export async function verifyOtpChallenge(challengeId:string,code:string){return db().begin(async(tx:any)=>{const rows=await tx`select * from customer_otp_challenges where id=${challengeId} for update`;const r=rows[0];if(!r||r.consumed_at||new Date(r.expires_at).getTime()<Date.now()||r.attempts>=5)return null;if(r.otp_hash!==otpH(code)){await tx`update customer_otp_challenges set attempts=attempts+1 where id=${challengeId}`;return null}await tx`update customer_otp_challenges set consumed_at=now() where id=${challengeId}`;return r})}
