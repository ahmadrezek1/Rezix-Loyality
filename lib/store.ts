...
export async function updateBusinessProfile(input:{businessId:string;name:string;industry:string;locationName?:string|null;street?:string|null;postalCode?:string|null;city?:string|null;country?:string;website?:string|null;counterMode?:'qr'|'pin'|'nfc';onboardingCompleted?:boolean}){
 const rows=await db()`update businesses set name=${input.name},industry=${input.industry},location_name=${input.locationName||null},street=${input.street||null},postal_code=${input.postalCode||null},city=${input.city||null},country=${input.country||'AT'},website=${input.website||null},counter_mode=${input.counterMode||'qr'},onboarding_completed=coalesce(${input.onboardingCompleted??null},onboarding_completed) where id=${input.businessId} returning *`;
...
export async function getManagerViewData(businessId:string,view:string,page=1){
 const businessPromise=getBusinessById(businessId);
 if(view==='customers'){
  const [business,customers,total]=await Promise.all([
   businessPromise,
  ...
  return {business,customers:customers.map(mapCustomer),totalCustomers:total[0]?.count??0,staff:[],staffCount:0,visitCount:0,visitsToday:0,activeCustomers:0,visitsMonth:0,rewards:0};
 }
 if(view==='friseure'){
  const [business,staff]=await Promise.all([businessPromise,db()`select id,name,email,active,email_verified_at from staff_users where business_id=${businessId} and role='friseur' order by created_at desc`]);
  return {business,staff,staffCount:staff.filter((x:any)=>x.active).length,customers:[],totalCustomers:0,visitCount:0,visitsToday:0,activeCustomers:0,visitsMonth:0,rewards:0};
 }
 if(view==='loyalty'){
  const business=await businessPromise;
  return {business,staff:[],staffCount:0,customers:[],totalCustomers:0,visitCount:0,visitsToday:0,activeCustomers:0,visitsMonth:0,rewards:0};
 }
 if(view==='settings'){
  const [business,visitCount]=await Promise.all([businessPromise,db()`select count(*)::int as count from visits where business_id=${businessId}`]);
  return {business,visitCount:visitCount[0]?.count??0,staff:[],staffCount:0,customers:[],totalCustomers:0,visitsToday:0,activeCustomers:0,visitsMonth:0,rewards:0};
 }
 // Overview aggregates related counters in two scans instead of many independent queries.
 const [business,staffCount,customerStats,visitStats]=await Promise.all([
  businessPromise,
  db()`select count(*)::int as count from staff_users where business_id=${businessId} and role='friseur' and active=true`,
  db()`select count(*) filter(where active=true)::int as total, count(*) filter(where active=true and last_visit_at>=now()-interval '30 days')::int as active, coalesce(sum(rewards_redeemed),0)::int as rewards from customers where business_id=${businessId}`,
