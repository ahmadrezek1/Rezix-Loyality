...
import { hexToRgbString, pemFromEnv, stableWalletToken } from './wallet-common';

type WalletCustomer={id:string;code:string;name:string;stamps:number};
type WalletBusiness={id:string;name:string;reward_target:number;reward_text:string;primary_color?:string|null};
export function appleWalletConfigured(){return Boolean(process.env.APPLE_PASS_TYPE_IDENTIFIER&&process.env.APPLE_TEAM_IDENTIFIER&&pemFromEnv('APPLE_WWDR_CERT')&&pemFromEnv('APPLE_PASS_CERT')&&pemFromEnv('APPLE_PASS_PRIVATE_KEY'))}
export async function createApplePass(customer:WalletCustomer,business:WalletBusiness){
 ...
 pass.backFields.push({key:'code',label:'Kundencode',value:customer.code}); pass.setBarcodes({format:'PKBarcodeFormatQR',message:customer.code,messageEncoding:'iso-8859-1',altText:customer.code});
