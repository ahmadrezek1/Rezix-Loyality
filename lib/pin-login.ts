...
function pinHash(pin:string,salt:string){return crypto.scryptSync(pepper(pin),salt,32).toString('hex');}
...
export async function loginWithPin(token:string,role:PinRole,pin:string){
 ...
 return database().begin(async tx=>{
  ...
  const hash=pinHash(pin,d.pin_salt);if(!crypto.timingSafeEqual(Buffer.from(hash),Buffer.from(d.pin_hash))){await tx`update trusted_pin_devices set attempts=attempts+1,revoked_at=case when attempts>=4 then now() else revoked_at end where token_hash=${d.token_hash}`;return null;}
