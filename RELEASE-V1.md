...
## Connection, verification and quick-login fixes

- Reproduced database `EMAXCONNSESSION` (15 session-pooler clients). Replaced the separate module pools with one shared two-connection pool per instance, with short idle expiry. Read-only dashboard loading succeeded afterward. The local connection uses the Supabase session pooler; production configuration and instance count still need monitoring.
- Existing managers in the configured database already have verified emails, so no new verification code is issued to them. Verification UI now explains this. Missing mail credentials no longer return false success; rejected requests have sanitized server diagnostics, and resend delivery failures are displayed. The user confirms production Resend is configured; live delivery was not exercised here.
- Trusted-device four-digit PIN is optional for Manager and Friseur, enabled after current-password confirmation in Schnell-Login einrichten. Device tokens use HttpOnly cookies, expire after 30 days, are scoped by role, and are revoked after five failures. Account deactivation, email verification and password changes are checked. Password login remains available.
