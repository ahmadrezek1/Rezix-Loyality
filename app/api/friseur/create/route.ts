...
export async function POST(req:Request){
  ...
  const f=await req.formData();const name=String(f.get('name')||'').trim();const email=String(f.get('email')||'').trim().toLowerCase();const password=String(f.get('password')||'');
  if(!name||!email.includes('@')||password.length<12)return NextResponse.redirect(new URL('/manager?friseurError=1',req.url),303);
  if(await staffEmailExists(email))return NextResponse.redirect(new URL('/manager?friseurExists=1',req.url),303);
  const business=await getBusinessById(s.businessId);if(!business)return NextResponse.redirect(new URL('/manager?billingBlocked=1',req.url),303);const entitlement=await assertCanCreateFriseur(business);if(!entitlement.ok)return NextResponse.redirect(new URL(`/manager?billingBlocked=1&billingReason=${entitlement.reason}&billingLimit=${entitlement.limit}`,req.url),303);
  const staffId=await createStaff({businessId:s.businessId,name,email,password,role:'friseur'});
  ...
  return NextResponse.redirect(new URL(`/manager?friseurAdded=1${sent?'':'&mailError=1'}`,req.url),303);
}
