...
export async function POST(req:Request){
 ...
 if(!business||!billingOperational(business))return NextResponse.json({error:'Salon-Tarif ist nicht aktiv'},{status:402});
 ...
 if(!customer)return NextResponse.json({error:'Kunde dieses Salons nicht gefunden'},{status:404});
