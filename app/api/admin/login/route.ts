import { adminPassword } from '@/lib/auth-store';
...
export async function POST(req:Request){
 ...
 if(!await consumeRateLimit('login-ip',requestIp(req),30,15))return NextResponse.redirect(new URL('/admin/login?blocked=1',req.url),303);
 ...
 if(!await isAdminEmailVerified(expected)){
   if(!await consumeRateLimit('admin-email',expected,3,15))return NextResponse.redirect(new URL('/admin/login?blocked=1',req.url),303);
