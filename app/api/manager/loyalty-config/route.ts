...
export async function POST(req:Request){
  ...
  const business=await getBusinessById(s.businessId); if(!business) return NextResponse.redirect(new URL('/manager/loyalty?configError=1',req.url),303);
  if(!billingOperational(business))return NextResponse.redirect(new URL('/manager/loyalty?billingBlocked=1',req.url),303);
  ...
  if(!Number.isInteger(rewardTarget)||rewardTarget<2||rewardTarget>30||!rewardText||!cardTitle||!cardSubtitle||!/^#[0-9A-Fa-f]{6}$/.test(primaryColor)||!['circle','rounded','square'].includes(stampShape)) return NextResponse.redirect(new URL('/manager/loyalty?configError=1',req.url),303);
  let customerDesign:CustomerDesign|undefined;
  if(f.has('backgroundMode')){
    const mode=String(f.get('backgroundMode'));const color=String(f.get('backgroundColor'));const gradientColor=String(f.get('gradientColor'));const overlay=Number(f.get('backgroundOverlay'));const position=String(f.get('backgroundPosition'));const size=String(f.get('backgroundSize'));
  ...
  try{
    ...
    if(customerDesign&&f.get('removeBackground')!=='on'&&background instanceof File&&background.size>0)customerDesign.imageUrl=await uploadSalonAsset(background,`${business.slug}/background`);
  ...
  }catch(e){console.error(e);return NextResponse.redirect(new URL('/manager/loyalty?configUpload=1',req.url),303)}
  try{await updateBusinessCardConfig({customerDesign,businessId:s.businessId,rewardTarget,rewardText,cardTitle,cardSubtitle,primaryColor,stampShape,logoUrl,stampUrl});}catch{return NextResponse.redirect(new URL('/manager/loyalty?configError=1',req.url),303);}
  ...
  return NextResponse.redirect(new URL('/manager/loyalty?configSaved=1',req.url),303);
}
