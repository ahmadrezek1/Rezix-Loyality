...
import { resolvePrivacyRequest } from '@/lib/store';
...
export async function POST(req:Request){
  ...
  if(!requestId||(decision!=='completed'&&decision!=='rejected'))return NextResponse.redirect(new URL('/manager?privacyError=1',req.url),303);
  const result=await resolvePrivacyRequest({requestId,businessId:s.businessId,managerId:s.sub,decision,note});
  if(result.kind!=='ok')return NextResponse.redirect(new URL('/manager?privacyError=1',req.url),303);
  ...
  return NextResponse.redirect(new URL('/manager?privacyUpdated=1',req.url),303);
}
