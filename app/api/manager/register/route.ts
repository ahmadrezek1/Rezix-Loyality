...
export async function POST(req:Request){
 ...
 try{
  ...
  const f=await req.formData();const industry=String(f.get('industry')||'other');const programType=String(f.get('programType')||'stamps') as 'stamps'|'points';const locationName=String(f.get('locationName')||'').trim();const street=String(f.get('street')||'').trim();const postalCode=String(f.get('postalCode')||'').trim();const city=String(f.get('city')||'').trim();const website=String(f.get('website')||'').trim();const cardTitle=String(f.get('cardTitle')||'Deine Treuekarte').trim();const name=String(f.get('name')||'').trim();const slug=String(f.get('slug')||'').trim();const managerName=String(f.get('managerName')||'').trim();const managerEmail=String(f.get('email')||'').trim().toLowerCase();const managerPassword=String(f.get('password')||'');const rewardText=String(f.get('rewardText')||'').trim();const rewardTarget=Number(f.get('rewardTarget'));
